...
const NavigationMobile = () => {
    ...
    const openModal = (e:React.MouseEvent<HTMLLIElement>,value:string) => {
            ...
            <nav>
                <ul className={styles.list} >
                    {linksLyst.map((item, index) => {
                        return (
                            <li key={index} className={styles.item} onClick={(e)=> openModal(e,item.value as string)} >
                                {item.value !== 'cities' && item.value !== 'numbers' && item.value !== 'languages'?<Link to={item.link}>{item.component}</Link>: <a className={styles.link} href="#">{item.component || item.link}</a>}
                            </li>
                        )
                    })}
